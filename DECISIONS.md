...
| D39 | Ritmo interno proporcional: cada cena é animada num relógio de projeto (`base`) e `PACING / base` estica/comprime todas as ações, molas, cursor e timelines GSAP (`useSceneTimeline` próprio, seek determinístico por frame); transições mantêm a velocidade real | Mudar só o fim da cena | Pedido do usuário: aumentar uma cena deixava tempo morto no fim. |
| D40 | Áudio refeito a partir da timeline: `SFX_CUES` e `MUSIC_SECTIONS` em `src/timeline.ts` usam os mesmos rótulos e o mesmo `stretch`; `scripts/audio/cues.mjs` gera `src/data/cues.json`; a síntese adapta harmonia/automação ao tamanho real de cada seção. `npm run audio` refaz tudo; o vídeo só toca o master se ele foi gerado para o `PACING` atual (e o `verify.mjs` falha caso contrário) | Master fixo | Pedido do usuário: música e SFX ressincronizados e "linkados" ao pacing. Revoga D38 (a camada de SFX no Remotion fica vazia; digitação fora do master por ora). |
